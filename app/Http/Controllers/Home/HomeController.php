...
class HomeController extends Controller
{
    ...
    public function welcome(Request $request)
    {
        ...
        $todaySum = DB::table('code_receiving_records')->where('user_id',$request->user()->id)->where('created_at', '>', date('Y-m-d', time()))->sum('amount');

        $monthSum = DB::table('code_receiving_records')->where('user_id',$request->user()->id)->where('created_at', '>', date('Y-m-01', time()))->sum('amount');

        $allSum = DB::table('code_receiving_records')->where('user_id',$request->user()->id)->sum('amount');
